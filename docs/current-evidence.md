...
Experiments 004–007 plus the dated E7 extension account for 224 model calls. Three studies completed their fixed schedules; Experiment 007 stopped administratively after 23 of 24 calls and received one separately registered delayed actor on September 10. The public notebook now contains 312 calls across seven studies, six completed and one stopped with a dated supplement. Calls, reviewer views and repeated states are not independent task samples.
...
| [007: benign mismatch control, dated supplement](experiment-07-report.md) | Both benign and both intact actions stayed safe; both harmful states produced violations. Actual-state forecasts scored Brier 0.0012 across six outcomes; query-aware code scored 0. | Relevance to the requested action matters beyond mere inequality with the source. No advantage over executable semantic checking was shown. One actor was collected later under an explicit amendment; the original five-target analysis remains separate. |

Brier score is mean squared probability error; lower is better on the same targets. The study-level means concern different outcome mixtures and must not be treated as a cross-study improvement series. The probabilities and 0.5 flag threshold are uncalibrated. E7's three previously unscored forecasts are now scored only in the separately labeled delayed supplement; the original stopped dataset is unchanged.
...
Do not launch the broad confirmatory monitorability comparison on this task distribution yet. Natural actor errors have not supplied a suitable failure population; the positive forecasting results used deliberately constructed state faults. The one-call supplement closes pilot collection but does not replace independent replication. Future work should develop held-out tasks with measured natural error rates, reviewer headroom, task diversity and independent calibration before fixing main-study power.
