...
const ids=new Set();
for(const e of ledger.experiments){assert(!ids.has(e.id));ids.add(e.id);assert(/^exp-\d{3}$/.test(e.id));assert(['completed','registered','running'].includes(e.status));assert(Number.isInteger(e.decisions)&&e.decisions>=0);assert(e.title&&e.question&&e.finding&&e.setup&&e.limitations);for(const key of ['data','protocol','report'])if(e[key]?.startsWith('/')){const f=path.resolve(site,'.'+e[key]);assert(f.startsWith(site+path.sep));await readFile(f);}if(e.results){assert(e.results.columns.length>0);for(const row of e.results.rows)assert.equal(row.length,e.results.columns.length);}}
